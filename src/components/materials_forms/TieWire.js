...
export default function SingleNameForm(props) {
  const url           = 'tiewires'
  const title         = 'Alambre de Amarre'
  const label         = 'alambre de amarre'
  const initialValue  = 'Alambre de amarre'
  ...
  const [serverState, setServerState] = useState();
        ...
        const handleOnSubmit = (values, {setSubmitting, resetForm}) => {
          const {name, knotsPerPound} = values
          api.post(`${url}`, {name, knotsPerPound})
            .then(response => {
