...
const Schemas = [
 ...
 function getSteps() {
   return ['Especificaciones de Losa Plana', 'Agregar costos de Concreto', 'Agregar costos de Armazón'];
 }

export default function ZapataDialogForm(props) {

  const url           = `budgets/${props.budgetId}/losasplanas`
          ...
          {({ submitForm, isSubmitting, setFieldValue, values, errors, touched }) => (
            ...
            {activeStep == 0 && (
                  ...
                  <Grid container spacing={3}>
                   ...
                   />
                   {errors.propTension && touched.propTension ? (
                     <div><Typography color='error' variant='caption'>{errors.propTension}</Typography></div>
                   ) : null}
                    </Grid>
